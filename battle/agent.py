...
class Agent(object):
    ...
    def train_a3c(self, max_episode_length, gamma, sess, coord, saver):
        ...
        with sess.as_default(), sess.graph.as_default():
            ...
            print("Time to imitation learning")
            self.imitation_learning(sess=sess, saver=saver, gamma=gamma)
    ...
    def play_game(self, sess, episode_num, verbose=True):
        ...
        for i in range(episode_num):
            ...
            while not self.env.is_episode_finished():
                game_state = self.env.get_state()
                game_vars = game_state.game_variables[:-1]
                state = [s, game_vars]
                ...
                if verbose:
                    ...
                    time.sleep(0.05)
            print('Agent: {}, End episode: {}, Total Kills: {}'.format(self.name, i, game_state.game_variables[-1]))
    ...
    def imitation_learning(self, sess, gamma, saver):
        with open(cfg.IMITATION_DATA_PATH % str(self.number), 'rb') as f:
            self.imitate_data = pickle.load(f)

        for _episode, _data in self.imitate_data.items():
            sess.run(self.update_local_ops)  # update local ops in every episode
            episode_reward = 0
            episode_buffer = []
            for _index in range(0, len(_data), 4):
                # get data
                _d = _data[_index:_index+4]
                s, a, g_vars = _data[_index][0], _data[_index][1], _data[_index][5:8]
                r, end, g_vars_after = sum([v[2] for v in _d]), _d[-1][4], _d[-1][8:11]
                # get value and reward
                a_dist, value = sess.run([self.local_AC_network.policy, self.local_AC_network.value], feed_dict={
                        self.local_AC_network.inputs: [s], self.local_AC_network.game_variables: [g_vars[:-1]]
                })
                reward = self.imitate_reward_function(game_vars=g_vars, game_vars_after=g_vars_after)
                if end:
                    s1 = s
                else:
                    s1 = _data[_index+4][3]
                episode_buffer.append([s, self.actions.index(a), reward, s1, end, value, np.array(g_vars[:-1])])

                if len(episode_buffer) == 64 and end is False:
                    v1 = sess.run(self.local_AC_network.value, feed_dict={
                        self.local_AC_network.inputs: [s],
                        self.local_AC_network.game_variables: [g_vars[:-1]]
                    })[0, 0]
                    l, v_l, p_l, e_l, g_n, v_n = self.infer(episode_buffer, sess, gamma, v1)
                    episode_buffer = []
                    sess.run(self.update_local_ops)
            ...
            if len(episode_buffer) != 0:
                l, v_l, p_l, e_l, g_n, v_n = self.infer(episode_buffer, sess, gamma, 0.0)
            saver.save(sess, self.model_path+'/model-'+str(_episode)+'.ckpt')
            print('Testing..... # {}'.format(_episode))
            self.play_game(sess=sess, episode_num=5, verbose=False)

        del self.imitate_data  # free memory
    ...
    @staticmethod
    def imitate_reward_function(game_vars, game_vars_after):
        ammo_delta = game_vars[0] - game_vars_after[0]

        health_delta = game_vars[1] - game_vars_after[1]

        kills_delta = game_vars[2] - game_vars_after[2]
